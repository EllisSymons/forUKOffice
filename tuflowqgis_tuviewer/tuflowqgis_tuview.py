...
class TuView(QDockWidget, Ui_Tuplot):
	...
	def projectCleared(self):
		...
		removeList = []
		for i in range(self.OpenResults.count()):
			item = self.OpenResults.item(i)
			removeList.append(item.text())
			
		self.tuResults.tuResults1D.removeResults(removeList)
	...
	def projectSaved(self):
		...
		self.saveTimer.timeout.connect(self.project.write)
	...
	def qgisConnect(self):
		...
		if not self.connected:
			...
			self.sliderTime.sliderReleased.connect(lambda: self.timeComboChanged(0))
	...
	def qgisDisconnect(self):
		...
		if self.connected:
			...
			self.sliderTime.sliderReleased.disconnect()
