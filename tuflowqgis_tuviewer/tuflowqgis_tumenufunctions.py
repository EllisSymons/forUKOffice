...
from tuflow.tuflowqgis_library import loadLastFolder, getResultPathsFromTCF, getScenariosFromTcf, getEventsFromTCF, tuflowqgis_find_layer, getUnit, getCellSizeFromTCF, getOutputZonesFromTCF, getPathFromRel
from tuflow.tuflowqgis_dialog import tuflowqgis_scenarioSelection_dialog, tuflowqgis_eventSelection_dialog, TuOptionsDialog, TuSelectedElementsDialog, tuflowqgis_meshSelection_dialog, TuBatchPlotExportDialog, TuUserPlotDataManagerDialog, tuflowqgis_outputZoneSelection_dialog
...
class TuMenuFunctions():
	...
	def load2dResults(self, **kwargs):
		...
		if ext.upper() == '.SUP':
			sups = self.resultsFromSuperFiles(inFileNames[0])
		...
		if not results2D and not results1D:
			...
			for i, m in enumerate(messages):
				if i == 0:
					...
					mes += '\n\n{0}'.format(m)
			QMessageBox.information(self.iface.mainWindow(), "TUVIEW", "Failed to load results from TCF\n\n{0}".format(mes))
	...
	def exportDataToClipboard(self):
		...
		if dataHeader is None or data is None:
			QMessageBox.critical(self.iface.mainWindow(), 'TuView', 'Error exporting data')
			return False
