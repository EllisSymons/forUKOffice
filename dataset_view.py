...
class DataSetItemDelegate(QStyledItemDelegate):
    def __init__(self, parent=None):
        ...
        self.pix_c  = QPixmap(os.path.dirname(__file__) + "\\icons\\icon_contours.png")
        self.pix_c0 = QIcon(self.pix_c).pixmap(self.pix_c.width(),self.pix_c.height(), QIcon.Disabled)
        self.pix_v = QPixmap(os.path.dirname(__file__) + "\\icons\\icon_vectors.png")
        self.pix_v0 = QPixmap(os.path.dirname(__file__) + "\\icons\\icon_vectors.png")
        self.pix_2nd = QPixmap(os.path.dirname(__file__) + "\\icons\\2nd_axis_2.png")
        self.pix_2nd0 = QPixmap(os.path.dirname(__file__) + "\\icons\\2nd_axis_20.png")
        ...
        self.pix_ts = QPixmap(os.path.dirname(__file__) + "\\icons\\results.png")
        self.pix_ts0 = QIcon(self.pix_ts).pixmap(self.pix_ts.width(), self.pix_ts.height(), QIcon.Disabled)
        self.pix_lp = QPixmap(os.path.dirname(__file__) + "\\icons\\results_lp.png")
        self.pix_lp0 = QIcon(self.pix_lp).pixmap(self.pix_lp.width(), self.pix_lp.height(), QIcon.Disabled)
        self.pix_max =  QPixmap(os.path.dirname(__file__) + "\\icons\\max.png")
        self.pix_max0 = QPixmap(os.path.dirname(__file__) + "\\icons\\max_inactive.png")
...
class DataSetView(QTreeView):
    ...
    def delayedMouseEvent(self):

        processed = False
        idx = self.indexAt(self.pressEvent.pos())
        if idx.isValid():
            vr = self.visualRect(idx)
            if self.itemDelegate().iconRect(vr, vr.right(), POS_2).contains(self.pressEvent.pos()):
            ...
            if self.itemDelegate().iconRect(vr, vr.right(), POS_MAX).contains(self.pressEvent.pos()):
                if self.model().index2item(idx).enabled:
        ...
        if not processed:
            if self.pressEvent.button() == Qt.LeftButton:
                self.leftClicked.emit({'modelIndex': idx, 'button': self.pressEvent.button()})

    def mousePressEvent(self, event):

        self.pressEvent = QMouseEvent(event)
        self.timer = QTimer()
        self.timer.setInterval(175)
        self.timer.setSingleShot(True)
        self.timer.timeout.connect(self.delayedMouseEvent)
        self.timer.start()
