...
def plotterMenu(bridge_editor, pos):
	...
	menu.popup(bridge_editor.gui.plotWdg.mapToGlobal(pos))
...
def xSectionTableMenu(bridge_editor, pos):
	...
	insertRowBefore_action.triggered.connect(lambda: insertRowBefore(bridge_editor, bridge_editor.gui.xSectionTable))
	insertRowAfter_action.triggered.connect(lambda: insertRowAfter(bridge_editor, bridge_editor.gui.xSectionTable))
	deleteRow_action.triggered.connect(lambda: deleteRow(bridge_editor, bridge_editor.gui.xSectionTable))
	menu.addAction(insertRowBefore_action)
	menu.addAction(insertRowAfter_action)
	menu.addAction(deleteRow_action)
	menu.popup(bridge_editor.gui.pierTable.mapToGlobal(pos))
...
def deckTableMenu(bridge_editor, pos):
	...
	insertRowAfter_action = QAction("Insert Row (after)", menu)
	deleteRow_action = QAction("Delete Row", menu)
	insertRowBefore_action.triggered.connect(lambda: insertRowBefore(bridge_editor, bridge_editor.gui.deckTable))
	insertRowAfter_action.triggered.connect(lambda: insertRowAfter(bridge_editor, bridge_editor.gui.deckTable))
	deleteRow_action.triggered.connect(lambda: deleteRow(bridge_editor, bridge_editor.gui.deckTable))
	menu.addAction(insertRowBefore_action)
	menu.addAction(insertRowAfter_action)
	menu.addAction(deleteRow_action)
	menu.popup(bridge_editor.gui.pierTable.mapToGlobal(pos))
...
def pierTableMenu(bridge_editor, pos):
	...
	insertRowAfter_action = QAction("Insert Row (after)", menu)
	deleteRow_action = QAction("Delete Row", menu)
	insertRowBefore_action.triggered.connect(lambda: insertRowBefore(bridge_editor, bridge_editor.gui.pierTable))
	insertRowAfter_action.triggered.connect(lambda: insertRowAfter(bridge_editor, bridge_editor.gui.pierTable))
	deleteRow_action.triggered.connect(lambda: deleteRow(bridge_editor, bridge_editor.gui.pierTable))
	menu.addAction(insertRowBefore_action)
	menu.addAction(insertRowAfter_action)
	menu.addAction(deleteRow_action)
	menu.popup(bridge_editor.gui.pierTable.mapToGlobal(pos))
...
def insertRowBefore(bridge_editor, table):
	...
	if currentRow is None:
		bridge_editor.gui.statusLog.insertItem(0, 'Error: No row selected')
	...
	if table == bridge_editor.gui.xSectionTable:
		bridge_editor.updateXsectionData()
	# Update spinbox
	dict = {bridge_editor.gui.xSectionTable: bridge_editor.gui.xSectionRowCount, bridge_editor.gui.deckTable: bridge_editor.gui.deckRowCount,
...
def export_csv(bridge_editor):
	...
	maxLen = 0
	for i in c:
		maxLen = max(maxLen, len(bridge_editor.gui.subplot.lines[i].get_data()[0]))
	# Get data
	for i, resultFile in enumerate(resultFiles):
		if i == 0:
			data = bridge_editor.gui.subplot.lines[c[i]].get_data()[0]  # write X axis first
			...
			dataX = bridge_editor.gui.subplot.lines[c[i]].get_data()[0]  # Write X axis again for new results
		...
		if i < len(c) - 1:  # isn't last result file
			for line in bridge_editor.gui.subplot.lines[c[i]:c[i + 1]]:
			...
			for line in bridge_editor.gui.subplot.lines[c[i]:]:
				dataY = line.get_data()[1]
				dataY = numpy.reshape(dataY, [len(dataY), 1])
				if len(dataY) < maxLen:
					diff = maxLen - len(dataY)
				...
				data = numpy.append(data, dataY, axis=1)
	# Save data out
	saveFile = QFileDialog.getSaveFileName(bridge_editor.gui, 'Save File', fpath)
	if len(saveFile) < 2:
		return
	else:
	...
	if saveFile is not None:
		try:
			...
			bridge_editor.gui.statusLog.insertItem(0, 'Error: Opening File for editing')
			bridge_editor.gui.statusLabel.setText('Status: Error')
			return
	bridge_editor.gui.statusLog.insertItem(0, 'Successfully exported csv')
